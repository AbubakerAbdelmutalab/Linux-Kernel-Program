...
#include <linux/sched.h>  // needed for jiffies
	    ...
	    loff_t *f_pos)
{
  ...
  /* YOUR CODE HERE */
  dev->flag = 1;
  wake_up_interruptible(&dev->dev_queue);
	     ...
	     loff_t *f_pos)
{
  ...
  unsigned long remaining_time = wait_event_interruptible_timeout(dev->dev_queue, dev->flag != 0, timer);
  ...
  retval = jiffies_to_msecs(remaining_time)/1000;
...
sleepy_init_module(void)
{
  ...
  for (i = 0; i < sleepy_ndevices; ++i) {
    ...
    init_waitqueue_head(&sleepy_devices[i].dev_queue);
